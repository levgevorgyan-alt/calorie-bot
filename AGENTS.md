...
scheduled water reminders. An AI input preflight catches typos and asks targeted
clarifying questions before estimating, so vague inputs like `eg` get a
"Did you mean **egg**?" follow-up instead of a bad estimate.
...
If your description is vague or has a typo (e.g. just `egg` or `eg`), the bot
runs an AI preflight that suggests the correction and asks a targeted
clarifying question — cooking method, quantity, with or without oil — before
estimating. Reply with the detail and the bot combines both messages for the
estimate.
...
- `/try <description>` - Preview calories for a meal without saving it to your log
...
- All times in the bot (water reminders, /history dates, etc.) are in UTC.
